...
async function fetchCloudHistory() {
    try {
        ...
        cloudHistory = {}; // Garante que é um objeto se falhar
    }
}
...
// NOVA FUNÇÃO: Faz merge com a cloud para não apagar o histórico antigo
function updateCloudHistory() {
    let hasChanges = false;
    const today = new Date();
    today.setHours(0, 0, 0, 0);

    // Fazemos uma cópia do histórico da cloud para adicionar, em vez de substituir do zero
    const mergedHistory = JSON.parse(JSON.stringify(cloudHistory || {}));
    ...
    // Se houve alterações (uma nova limpeza gravada), envia para a cloud
    if (hasChanges) {
        saveToCloudHistory(mergedHistory);
        cloudHistory = mergedHistory; // Atualiza a variável local também
    }
}
